...
module.exports.KbCountBig=function(type,inputC,inputK){
    //0为正度，1为大度,默认为正度
    var zc=1092;
    var zk=787;
    if(type==1){
    ...
    //需计算的长宽高
    var c=Number(inputC>inputK?inputC:inputK);
    var k=Number(inputC>inputK?inputK:inputC);
    //最终开别结果
    var kbEnd1;
    var kbEnd2;
    if (c>=k){//测试数据250 220
...
module.exports.DJSXkb=function(name,ccc,c,kkk,k,typeName){
    //长宽校正
    /*let ccc=iccc>ikkk?iccc:ikkk;
    let kkk=iccc>ikkk?ikkk:iccc;
    let c=ic>ik?ic:ik;
    let k=ic>ik?ik:ic;*/
