...
<script>
import TopBoxMenu from '../components/TopBoxMenu.vue'
...
export default {
  ...
  computed:{
    ...
    bottomCardboardLong:function(){
      return Number(this.long)-Number(this.thick*2)-this.bottomBack+Number(this.height-this.bThick)*2;
    },
    bottomCardboardWide:function(){
      return Number(this.wide)-Number(this.thick*2)-this.bottomBack+Number(this.height-this.bThick)*2;
    },
    topcolorsurfaceLong:function(){
      return Number(this.long)+Number(this.topHeight*2)+Number(this.thick*2+this.curling*2+1);
    },
    topcolorsurfaceWide:function(){
      return Number(this.wide)+Number(this.topHeight*2)+Number(this.thick*2+this.curling*2+1);
    },
    bottomColorSurfaceLong:function(){
      return Number(this.long-this.bThick*2-this.bottomBack)+Number(this.height*2-this.thick*2)+Number(this.bottomCurling*2+this.bThick*2+1);
    },
    bottomColorSurfaceWide:function(){
      return Number(this.wide-this.bThick*2-this.bottomBack)+Number(this.height*2-this.thick*2+this.bottomCurling*2)+Number(this.bThick*2+1);
    }
  },
  methods:{
    ...
    isBottomCurlingBottom:function(){
      if(this.isBottomCurlingBottomSwitch){
        this.bottomCurling=this.height;
      }else{
      ...
      js_CountPrice.ProcessPromise(boxName,this.quantity).then(value=>{
        ...
        if(this.isCardboard=='含纸板'){//判断是否含纸板
          ...
          return js_CountPrice.CardboardPromise(this.topCardboardLong+this.changeNumber,this.topCardboardWide+this.changeNumber,this.cardboard,this.thick,true).then(value=>{
            ...
            return js_CountPrice.CardboardPromise(this.bottomCardboardLong+this.changeNumber,this.bottomCardboardWide+this.changeNumber,this.BottomCardboard,this.bThick,true).then(value=>{
