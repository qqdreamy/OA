...
<div>
  ...
  <el-dialog title="天地盖-盖到底" v-model="dialogTableVisible" size="large">
    ...
    <el-col :span="20">
    底(外空)：{{topInnerLong-bottomBack}}*{{topInnerWide-bottomBack}}*{{bottomHeight}}
    底(内空)：{{long-thick*4-bottomBack}}*{{wide-thick*4-bottomBack}}*{{bottomHeight-thick}}
    </el-col>
...
export default {
  ...
  computed:{
    ...
    bottomColorSurfaceLong:function(){
      return (this.long-this.thick*2-2)+(this.bottomHeight)*2+this.curling*2+this.thick*2+1;
    },
    bottomColorSurfaceWide:function(){
      return (this.wide-this.thick*2-2)+(this.bottomHeight)*2+this.curling*2+this.thick*2+1;
    }
  },
  methods:{
    test:function(e){
