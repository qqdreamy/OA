...
<div>
  ...
  <dialogPrice v-on:closePrice="closePrice" :dialogPriceVisible="dialogPriceVisible">
     <p slot="list"> 
      ...
      裱瓦：{{this.boxPrice.CorrugatedMount}}
     </p>
...
export default {
  ...
  computed:{
    ...
    ExpandWide:function(){
      return this.height+this.wide+30+this.wide/2+30
    }
  },
  methods:{
    ...
    CountPrice:function(){
      js_CountPrice.KaHePromise(this.ExpandLong,this.ExpandWide,this.quantity).then(value=>{
        ...
        if(this.isCorrugated){
          return js_CountPrice.CorrugatedMount(this.ExpandLong,this.ExpandWide,'空缺').then(value=>{
